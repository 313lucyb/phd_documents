...
predictions_fpsm<-function(prediction_datasets, model, landmark_times, covariates, PW){
  ...
  #NEW-START
  base <- data.frame(age=0, 
                    sex=0, 
                    disease_duration=0, 
                    bmi=0, 
                    patient_disease = 0,
                    clinician_disease = 0)
  list <- list(plot(model, type = "surv", newdata = base))
  basesurv <-list[[1]]$Estimate
  times <- lapply(list, attributes)[[1]]$newdata$Time
  base_surv_data <- data.frame(times = times, basesurv = basesurv)
  base_surv_vec <-0
  for (j in (1:length(landmark_times))){
    base_surv_vec[j] = base_surv_data[which.min(abs(j-base_surv_data$times)),]$basesurv
  }
  base_surv_vec <- c(1, base_surv_vec)
  #NEW-END
  for (i in 1:length(prediction_datasets)){
    ...
    start_time <- Sys.time()
    #NEW
    predictions <- (base_surv_vec[i+1]/base_surv_vec[i])**(exp(dataset$fpsm_lp))
    ...
    #NEW
    dataset$fpsm_pred <- as.numeric(predictions)
...
predictions_tdcm<-function(prediction_datasets, model, landmark_times, covariates, PW){
  ...
  basesurv <-summary(survfit(model, newdata = base))$surv
  times <- summary(survfit(model, newdata = base))$time
  base_surv_data <- data.frame(times = times, basesurv = basesurv)
  base_surv_vec <-0
  for (j in (1:length(landmark_times))){
    base_surv_vec[j] = base_surv_data[which(base_surv_data$times==max(
      base_surv_data$times[(base_surv_data$times <= j)])),]$basesurv
  }
  base_surv_vec <- c(1, base_surv_vec)
  #NEW-END
  for (i in 1:length(prediction_datasets)){
    ...
    #)
    #NEW
    tdcm_pred1 <- (base_surv_vec[i+1]/base_surv_vec[i])**(exp(dataset$tdcm_lp))
