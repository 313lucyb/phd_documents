...
base<- data.frame(age=0, pgen=0, trtment="1", 
                  disdur=as.integer(0), previous_dmards =as.integer(0), lung=0,
                  diabetes=0, bmi=0, renal=0, steroids=as.integer(0), ovmean=0, 
                  smoke="0", dascore=0)


basesurv <-summary(survfit(m1.1, newdata=base))$surv
times <- summary(survfit(m1.1, newdata=base))$time
base_surv_data <- data.frame(times=times, basesurv=basesurv)
basesurv05<-base_surv_data[which(base_surv_data$times==max(
  base_surv_data$times[base_surv_data$times <= 0.5])),]$basesurv
basesurv1<-base_surv_data[which(base_surv_data$times==max(
  base_surv_data$times[base_surv_data$times <= 1])),]$basesurv
basesurv15<-base_surv_data[which(base_surv_data$times==max(
  base_surv_data$times[base_surv_data$times <= 1.5])),]$basesurv
basesurv2<-base_surv_data[which(base_surv_data$times==max(
  base_surv_data$times[base_surv_data$times <= 2])),]$basesurv
basesurv25<-base_surv_data[which(base_surv_data$times==max(
  base_surv_data$times[base_surv_data$times <= 2.5])),]$basesurv
basesurv3<-base_surv_data[which(base_surv_data$times==max(
  base_surv_data$times[base_surv_data$times <= 3])),]$basesurv
basesurv35<-base_surv_data[which(base_surv_data$times==max(
  base_surv_data$times[base_surv_data$times <= 3.5])),]$basesurv
basesurv4<-base_surv_data[which(base_surv_data$times==max(
  base_surv_data$times[base_surv_data$times <= 4])),]$basesurv
...
lp0<-as.vector(t(lp0))
test0TDCM <- test0
test0TDCM$timevent <- rep(1, length(test0TDCM$studyno))
...
test0$surv <- base_surv_vec[2]**exp(lp0)
#test0$surv3 <- predict(m1.1, newdata = test0TDCM, type = "surv")
#test0$diff <- test0$surv - test0$surv2
#hist(test0$diff)
end_time0 <- Sys.time()
...
test1$surv <- (base_surv_vec[3]/base_surv_vec[1])**(exp(lp1))
end_time1 <- Sys.time()
test1$pred <- 1 - test1$surv
...
test2$surv <- (base_surv_vec[4]/base_surv_vec[2])**(exp(lp2))
end_time2 <- Sys.time()
test2$pred <- 1 - test2$surv
...
test3$surv <- (base_surv_vec[5]/base_surv_vec[3])**(exp(lp3))
end_time3 <- Sys.time()
test3$pred <- 1 - test3$surv
...
test4$surv <- (base_surv_vec[6]/base_surv_vec[4])**(exp(lp4))
end_time4 <- Sys.time()
test4$pred <- 1 - test4$surv
...
test5$surv <- (base_surv_vec[7]/base_surv_vec[5])**(exp(lp5))
end_time5 <- Sys.time()
test5$pred <- 1 - test5$surv
...
test6$surv <- (base_surv_vec[8]/base_surv_vec[6])**(exp(lp6))
end_time6 <- Sys.time()
test6$pred <- 1 - test6$surv
...
bootvadTDCM<-function(data, B, seed){
  ...
  for (j in 1:B){
    ...
    tdcm.b <- coxph(Surv(start, stop, serinf) ~ ovmean + dascore +trtment + pgen + age + 
                bmi + renal + lung + diabetes + smoke + steroids + 
                previous_dmards + disdur, data=dat.b)

    print(summary(tdcm.b))
    base<- data.frame(age=0, pgen=0, trtment="1", 
                      disdur=as.integer(0), previous_dmards =as.integer(0), lung=0,
                      diabetes=0, bmi=0, renal=0, steroids=as.integer(0), ovmean=0, 
                      smoke="0", dascore=0)
    ...
    basesurv <-summary(survfit(tdcm.b, newdata=base))$surv
    times <- summary(survfit(tdcm.b, newdata=base))$time
    base_surv_data <- data.frame(times=times, basesurv=basesurv)
    basesurv05<-base_surv_data[which(base_surv_data$times==max(
      base_surv_data$times[base_surv_data$times <= 0.5])),]$basesurv
    basesurv1<-base_surv_data[which(base_surv_data$times==max(
    ...
    m1coef<-as.numeric(tdcm.b$coefficients)
    print(m1coef)
    ...
    print("step")
    print(length(val0$studyno))
    
    # Linear predictors
    lpcov1<-val1 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                              pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                              steroids, previous_dmards, disdur)
    lpmat1<-as.matrix(lpcov1)
    lp1<-m1coef %*% t(lpmat1)
    lp1<-as.vector(t(lp1))
    
    lpcov2<-val2 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                            pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                            steroids, previous_dmards, disdur)
    lpmat2<-as.matrix(lpcov2)
    lp2<-m1coef %*% t(lpmat2)
    lp2<-as.vector(t(lp2))
    
    lpcov3<-val3 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                            pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                            steroids, previous_dmards, disdur)
    lpmat3<-as.matrix(lpcov3)
    lp3<-m1coef %*% t(lpmat3)
    lp3<-as.vector(t(lp3))
    
    lpcov4<-val4 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                            pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                            steroids, previous_dmards, disdur)
    lpmat4<-as.matrix(lpcov4)
    lp4<-m1coef %*% t(lpmat4)
    lp4<-as.vector(t(lp4))
    
    lpcov5<-val5 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                            pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                            steroids, previous_dmards, disdur)
    lpmat5<-as.matrix(lpcov5)
    lp5<-m1coef %*% t(lpmat5)
    lp5<-as.vector(t(lp5))
    
    lpcov6<-val6 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                            pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                            steroids, previous_dmards, disdur)
    lpmat6<-as.matrix(lpcov6)
    lp6<-m1coef %*% t(lpmat6)
    lp6<-as.vector(t(lp6))
    ...
    val1$surv<-(base_surv_vec[3]/base_surv_vec[1])**(exp(lp1))
    val2$surv<-(base_surv_vec[4]/base_surv_vec[2])**(exp(lp2))
    val3$surv<-(base_surv_vec[5]/base_surv_vec[3])**(exp(lp3))
    val4$surv<-(base_surv_vec[6]/base_surv_vec[4])**(exp(lp4))
    val5$surv<-(base_surv_vec[7]/base_surv_vec[5])**(exp(lp5))
    val6$surv<-(base_surv_vec[8]/base_surv_vec[6])**(exp(lp6))
    ...
    # Linear predictors 
    lpcov1od <-od1 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                            pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                            steroids, previous_dmards, disdur)
    lpmat1od<-as.matrix(lpcov1od)
    lp1od<-m1coef %*% t(lpmat1od)
    lp1od<-as.vector(t(lp1od))
    
    lpcov2od <-od2 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                              pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                              steroids, previous_dmards, disdur)
    lpmat2od<-as.matrix(lpcov2od)
    lp2od<-m1coef %*% t(lpmat2od)
    lp2od<-as.vector(t(lp2od))
    
    lpcov3od <-od3 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                              pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                              steroids, previous_dmards, disdur)
    lpmat3od<-as.matrix(lpcov3od)
    lp3od<-m1coef %*% t(lpmat3od)
    lp3od<-as.vector(t(lp3od))
    
    lpcov4od <-od4 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                              pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                              steroids, previous_dmards, disdur)
    lpmat4od<-as.matrix(lpcov4od)
    lp4od<-m1coef %*% t(lpmat4od)
    lp4od<-as.vector(t(lp4od))
    
    lpcov5od <-od5 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                              pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                              steroids, previous_dmards, disdur)
    lpmat5od<-as.matrix(lpcov5od)
    lp5od<-m1coef %*% t(lpmat5od)
    lp5od<-as.vector(t(lp5od))
    
    lpcov6od <-od6 %>% select(ovmean, dascore, trt2, trt4, trt1561, trt10060, 
                              pgen, age, bmi, renal, lung, diabetes, smoke1, smoke2,
                              steroids, previous_dmards, disdur)
    lpmat6od<-as.matrix(lpcov6od)
    lp6od<-m1coef %*% t(lpmat6od)
    lp6od<-as.vector(t(lp6od))
    
    # Survival predictions - original data
    
    od0$surv<-1-risk(tdcm.b, newdata=od0, time = 1)
    od1$surv<-(base_surv_vec[3]/base_surv_vec[1])**(exp(lp1od))
    od2$surv<-(base_surv_vec[4]/base_surv_vec[2])**(exp(lp2od))
    od3$surv<-(base_surv_vec[5]/base_surv_vec[3])**(exp(lp3od))
    od4$surv<-(base_surv_vec[6]/base_surv_vec[4])**(exp(lp4od))
    od5$surv<-(base_surv_vec[7]/base_surv_vec[5])**(exp(lp5od))
    od6$surv<-(base_surv_vec[8]/base_surv_vec[6])**(exp(lp6od))
...
write.table(TDCM.OPT, file = "tdcmOPT.csv", sep = ",", col.names = NA,
            qmethod = "double")
x<-read.table("tdcmOPT.csv", header = TRUE, sep = ",", row.names = 1)
head(x)
...
plot(survfit(m1), xlab="Time since baseline (years)", ylab="Suvival probability", cex.lab=1.2, lwd=2, col = alpha(cols[11], 0.3), ylim=c(0.85,1))
