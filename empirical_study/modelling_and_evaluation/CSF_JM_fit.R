...
JMdata <- read.dta("JMlong_haq.dta")
...
set.seed(310394)
lmeFit<- lme(ovmean~ns(pyears, knots=2), data = JMdata, 
             random = ~ ns(pyears,knots=2) | groupid)
...
summary(jointFit)
saveRDS(jointFit, "./jointmodelresults/haq1.rds")
