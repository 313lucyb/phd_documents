...
long_data <- long_data %>% group_by(groupid) %>% mutate(dascore = na.locf(ovmean))

# fitted model

jointFit <- readRDS("~/jointmodelresults/haq4.rds")

file.names<-c("cal40.csv","cal41.csv","cal42.csv","cal43.csv", "cal44.csv", "cal45.csv", "cal46.csv")
...
JMTA<- foreach(i=1:7, .combine=rbind, .packages = c("ipred", "splines", 
                                                    "survival", "JMbayes", "dplyr", "tidyr")) %dopar% {
                                                      JMperf(LT[i], HT[i], jointFit, long_data, i, "dascore", 310394+i)
                                                    }
stopCluster(cl)
print(JMTA)
write.table(JMTA, file = "./JM4_TA.csv", sep = ",", col.names = NA,
            qmethod = "double")
